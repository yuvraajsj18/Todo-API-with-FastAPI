...
def test_create_user(db: Session):
    ...
    user = crud.users.create_user(db, user=user_in)
...
def test_not_authenticate_user(db: Session):
    ...
    user_in = UserCreate(username=username, password=password)
    user = crud.users.create_user(db, user=user_in)
    assert not crud.users.authenticate_user(db, username=username, password="wrong")
    assert not crud.users.authenticate_user(db, username="wrong", password=password)
    assert not crud.users.authenticate_user(db, username="wrong", password="wrong")
...
def test_get_user_by_id(db: Session):
    ...
    user_in = UserCreate(username=username, password=password)
    user = crud.users.create_user(db, user=user_in)
    user_out = crud.users.get_user(db, user_id=user.id)
    assert user_out
...
def test_get_user_by_username(db: Session):
    ...
    user_in = UserCreate(username=username, password=password)
    user = crud.users.create_user(db, user=user_in)
    user_out = crud.users.get_user_by_username(db, username=username)
    assert user_out
    assert user_out.username == user.username
    assert user_out.hashed_password == user.hashed_password
...
def test_get_users(db: Session):
    ...
    user_in = UserCreate(username=username, password=password)
    user = crud.users.create_user(db, user=user_in)
    user_out = crud.users.get_users(db)
    print(user_out)
    assert user_out
